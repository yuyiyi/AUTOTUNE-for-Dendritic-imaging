...
if handles.withmask == 1 && ~isempty(handles.im_mask)
    ...
    if ~isempty(handles.dendriteROI_mask) && ~isempty(handles.roi_seed_master) && handles.shaft_flag == 1
        handles = call_autoshaft(handles, 0);
        ifshaft = 1;
    elseif ~isempty(handles.dendriteROI_mask) && ~isempty(handles.roi_seed_master) && handles.shaft_flag == 2
        handles = call_autoshaftwholeDendr(handles, 0);
        ifshaft = 1;
    end
else
...
Save_SingleData(handles, f_wait)
