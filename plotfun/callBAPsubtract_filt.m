function [handles] = callBAPsubtract_filt(handles, dataID)
if nargin<2
    dataID = 1;
end
f_wait = waitbar(0.5, 'Auto processing Backpropagation removal');
scrsz = handles.scrsz;
handles = call_autoBAP_filt(handles);
traceplot22(handles)
close(f_wait);
delete(f_wait);
if handles.openGUI==0
    ...
    if strcmp(comIn, 'y')
        ...
        [handles.spine_BAPremoval_coef, handles.spine_trace_BAPremoval] = ...
            BAPupdate_CMD(handles.spine_filt, handles.spine_BAPremoval_coef,...
        ...
        [handles.shaft_BAPremoval_coef, handles.shaft_trace_BAPremoval] = ...
            BAPupdate_CMD(handles.shaft_filt, handles.shaft_BAPremoval_coef,...
    ...
    if strcmp(comIn, 'Yes')
        if ~isempty(handles.spineROI)
            spID = handles.spine_title;
        [handles.spine_BAPremoval_coef, handles.spine_trace_BAPremoval] = ...
            BAPupdate_GUI(handles.spine_filt, handles.spine_BAPremoval_coef,...
            handles.spine_trace_BAPremoval,handles.spine_BAP_current, scrsz, 'Spine', spID);
        end
        if ~isempty(handles.dend_shaft)
            shID = handles.shaft_title;
        [handles.shaft_BAPremoval_coef, handles.shaft_trace_BAPremoval] = ...
            BAPupdate_GUI(handles.shaft_filt, handles.shaft_BAPremoval_coef,...
            handles.shaft_trace_BAPremoval,handles.shaft_BAP_current, scrsz, 'Shaft', shID);
        end
        traceplot22(handles)
