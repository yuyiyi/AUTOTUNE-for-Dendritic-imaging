...
tic
RegPara.NimgFirstRegistration  = max(min(RegPara.FrameNoiniAlign, RegPara.Imagelength(1)*0.2), 1);
...
%         end
        ...
        id_badframes = intersect(find(Corr_raw <= RegPara.lowCorr),...
                    find( (abs(dds(:,1)) > RegPara.maxDispPerFrame(1))+...
                    (abs(dds(:,2)) > RegPara.maxDispPerFrame(2)) > 0));
        ...
        if dregmode > 0
            ...
            if handles.savesubsampletif == 1
                [samplelength, samplefileid] = ...
                    savesubtif(handles, dregbatch(:,:,1:savegrad:length(indxr)), RegPara, samplefileid, ...
