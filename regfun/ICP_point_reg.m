function [R, t, iter_err, reg_pointqueary] = ...
    ICP_point_reg(reg_points, reg_pointqueary, ops)
...
distTh0 = maxdist;
    ...
    %         distTh = maxdist;
        ...
        if flag > 3
            flag = 0;
            distTh = max(mindist, distTh*0.9);
            distTh0 = distTh;
        end
    %     if abs(e0-e1) < 1
    %         distTh = max(distTh - 10, 10);
    %     end
    %     end
    ...
    %     [~, ii, ~] = unique(data_match(:,1));
        ...
        reg_pointqueary = bsxfun(@plus, reg_pointqueary, t1);
        R = R1*R;
        t = R1*t + t1;    
        c1 = length(ii);        
        e0 = e1;
        ...
        if dispreg
            figure(1),
            clf('reset')
    %         imshowpair(I2, I1,'Scaling','joint')
            hold on, plot(reg_points(1,:), reg_points(2,:), '.y')
            hold on, plot(reg_points(1,data_match(:,1)), reg_points(2,data_match(:,1)), 'oy')
            hold on, plot(reg_pointqueary(1,:), reg_pointqueary(2,:), '.r')
...
%             F(iter) = getframe(gcf);
    ...
    e1 = 0; 
    while abs(e1-e0)>tot && iter < maxIter-10
        e0 = e1; 
        [idx, D] = dsearchn(reg_points', tri, reg_pointqueary');
        data_match = [idx, [1 : size(reg_pointqueary,2)]', D];    
    %     distTh = mindist;
